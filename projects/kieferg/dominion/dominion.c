...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		callAdventurer(cardDrawn, drawntreasure, state, currentPlayer, temphand, z);
      return 0;
    ...
    case minion:
      callMinion(state, handPos, currentPlayer, choice1, choice2);
      return 0;
    ...
    case cutpurse:
		callCutPurse(currentPlayer, state, handPos);
      return 0;
...
/* Refactored Code - Place cardEffect switch statements here */
//	- adventurer	-	line 669
//	- smithy		-	line 814
//	- cutpurse		-	line 1082
//	- baron			-	line 827
...
void callAdventurer(int cardDrawn, int drawntreasure, struct gameState *state, int currentPlayer, int temphand[], int z)
{
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
	  ...
	  // The line below has been changed to increment by one instead of decrement
...
{
	//+3 Cards
	// A bug is introduced below, where originally: i < 3 is now i <= 3 
    for (int i = 0; i <= 3; i++)
	{
	  drawCard(currentPlayer, state);
	}
	  ...
	  if (i != currentPlayer)
	      ...
	      for (int j = 0; j < state->handCount[i]; j++)
		{
		  if (state->hand[i][j] == copper)
		    {
		      // A bug is introduced below, the trashFlag was originally set to 0 and is now 1
	...
	while(card_not_discarded){
	  if (state->hand[currentPlayer][p] == estate){//Found an estate card!
	    ...
	    if (supplyCount(estate, state) > 0){
	      ...
	      // A bug has been introduced below. Equality was originally set to 0 and is now 1.
